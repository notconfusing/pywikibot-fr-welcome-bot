...
netext = {
    ...
    'wikipedia': {
        ...
        'en': '{{subst:welcome}} %s',
        'fa': '{{جا:خوشامد}} %s',
        'fr': '{{subst:Utilisateur:Darkoneko/Message automatique de bienvenue|%s}} [[user:%s|%s]] ([[user talk:%s|discuter]])',
        'ga': '{{subst:fáilte}} %s',
        'he': '{{ס:ברוך הבא}} %s',
        'hr': '{{subst:dd}} %s',
        'id': '{{subst:sdbot2}}\n%s',
...
random_sign = {
    ...
    'fr': 'Projet:Aide et accueil/Signatures',
            ...
            for users in self.parseNewUserLog():
                ...
                if users.editCount() >= globalvar.attachEditCount:
                    ...
                    if ustp.exists():
                        ...
                        if globalvar.randomSign:
                            if self.site.family.name == 'wikipedia' and self.site.code == 'fr':
                                x_username = choice(self.defineSign())
                                welcome_text = welcome_text % (x_username, x_username, x_username, x_username) + timeselected
                            else:
